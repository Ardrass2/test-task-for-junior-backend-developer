...
func NewRouter(
	...
	periodicTaskHandler *httphandlers.PeriodicTaskHandler,
	...
	api.HandleFunc("/periodic-tasks", periodicTaskHandler.Create).Methods(http.MethodPost)
	api.HandleFunc("/periodic-tasks", periodicTaskHandler.List).Methods(http.MethodGet)
	api.HandleFunc("/periodic-tasks/{id:[0-9]+}", periodicTaskHandler.GetByID).Methods(http.MethodGet)
	api.HandleFunc("/periodic-tasks/{id:[0-9]+}", periodicTaskHandler.Update).Methods(http.MethodPut)
	api.HandleFunc("/periodic-tasks/{id:[0-9]+}", periodicTaskHandler.Delete).Methods(http.MethodDelete)
